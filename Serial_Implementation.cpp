...
public:
    ...
    void removeEdge(int u, int v) {
        ...
        for (auto it = A.begin(); it != A.end(); ++it) {
            if (it->first == v) { A.erase(it); break; }
        }
        auto &B = adj[v];
        for (auto it = B.begin(); it != B.end(); ++it) {
            if (it->first == u) { B.erase(it); break; }
        }
    }
};
...
Graph readGraph(const std::string& fname) {
    ...
    Graph G(maxv+1);
    for (auto &e: edges) G.addEdge(e.u,e.v,e.w);
...
void dijkstra(const Graph &G, int src,
    ...
    while(!pq.empty()) {
        ...
        for(auto &pr: G.adj[u]) {
            int v=pr.first; real w=pr.second;
            real nd=d+w;
            if (nd<dist[v]) {
                dist[v]=nd;
                parent[v]=u;
...
void ProcessCE(const Graph &G,
    ...
    for(auto &e:Del){
        ...
        if(parent[u]==v||parent[v]==u){
            ...
            dist[y]=INF; AD.insert(y); A.insert(y);
        }
    }
    for(auto &e:Inst){
        int u=e.u,v=e.v; real w=e.w;
        int x=(dist[u]>dist[v]?v:u), y=(x==u?v:u);
        if(dist[x]<INF && dist[x]+w<dist[y]){
            dist[y]=dist[x]+w; parent[y]=x; A.insert(y);
        }
    }
}
...
void UpdateAffectedVertices(const Graph &G,
    ...
    while(!AD.empty()){
        std::set<int> nextD;
        for(int v:AD){
            for(int c=0;c<N;++c) if(parent[c]==v){
                dist[c]=INF; nextD.insert(c); A.insert(c);
            }
        }
        AD.swap(nextD);
    }
    ...
    while(!A.empty()){
        ...
        for(int v:A){
            for(auto &pr:G.adj[v]){
                int n=pr.first; real w=pr.second;
                if(dist[v]<INF && dist[v]+w<dist[n]){
                    dist[n]=dist[v]+w; parent[n]=v; nextA.insert(n);
                }
                if(dist[n]<INF && dist[n]+w<dist[v]){
                    dist[v]=dist[n]+w; parent[v]=n; nextA.insert(v);
                }
            }
        }
        A.swap(nextA);
...
int main(){
    ...
    std::vector<real> dist2; std::vector<int> parent2;
    ...
    if(match) std::cout<<"Dynamic update correct\n";
    else std::cout<<"Dynamic update incorrect\n";

    // 6) output first 100 values
    std::ofstream fout("dynamic_sssp_output.txt");
    fout<<std::fixed;
    for(int i=0;i<std::min(G.n,100);++i){
        if(dist[i]==INF) fout<<i<<" unreachable\n";
        else             fout<<i<<" "<<dist[i]<<"\n";
    }
    return 0;
}